...
library AlienMetadataSvg {
	...
	function generateSVGofTokenById(string memory name) internal pure returns (string memory) {
	...
	function tokenURI(uint256 tokenId) internal pure returns (string memory) {
		...
		string memory image = Base64.encode(bytes(generateSVGofTokenById(name)));
		return string(
			abi.encodePacked(
